...
bool LSTMTrainer::DeSerialize(TFile* fp) {
  ...
  if (fp->FRead(&learning_iteration_, 1) != 1) {
  ...
  if (fp->FRead(&prev_sample_iteration_, 1) != 1)
    return false;
  if (fp->FRead(&perfect_delay_, 1) != 1) return false;
  if (fp->FRead(&last_perfect_training_iteration_, 1) != 1)
    return false;
  for (int i = 0; i < ET_COUNT; ++i) {
    if (!error_buffers_[i].DeSerialize(fp)) return false;
  }
  if (fp->FRead(error_rates_, 1) != 1) return false;
  if (fp->FRead(&training_stage_, 1) != 1)
    return false;
  uinT8 amount;
  if (fp->FRead(&amount, 1) != 1) return false;
  if (amount == LIGHT) return true;  // Don't read the rest.
  if (fp->FRead(&best_error_rate_, 1) != 1)
    return false;
  if (fp->FRead(best_error_rates_, 1) != 1)
    return false;
  if (fp->FRead(&best_iteration_, 1) != 1)
    return false;
  if (fp->FRead(&worst_error_rate_, 1) != 1)
    return false;
  if (fp->FRead(worst_error_rates_, 1) != 1)
    return false;
  if (fp->FRead(&worst_iteration_, 1) != 1)
    return false;
  if (fp->FRead(&stall_iteration_, 1) != 1)
    return false;
  ...
  if (fp->FRead(&improvement_steps_, 1) != 1)
    return false;
  return true;
}
