...
void POLY_BLOCK::compute_bb() {  // constructor
  ...
  do {
    ...
    if (pos.y() < botleft.y()) botleft = ICOORD(botleft.x(), pos.y());
    if (pos.x() > topright.x()) topright = ICOORD(pos.x(), topright.y());
    if (pos.y() > topright.y()) topright = ICOORD(topright.x(), pos.y());
    pts.forward();
  } while (!pts.at_first());
...
ICOORDELT_LIST* PB_LINE_IT::get_line(int16_t y) {
  ...
  if (!r.empty()) {
    r.sort(lessthan);
    for (r.mark_cycle_pt(); !r.cycled_list(); r.forward()) x = r.data();
