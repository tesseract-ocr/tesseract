  ...
  if (blobcount > 1 && cblob_ptr != nullptr) {
    ...
    for (blobindex = blobcount - 1, rightx = box.right(); blobindex >= 0;
      ...
      if (ymin < ymax) {
        ...
        if (leftx < box.left()) leftx = box.left();  // clip to real box
...
int BLOBNBOX::GoodTextBlob() const {
  ...
  for (int dir = 0; dir < BND_COUNT; ++dir) {
    ...
    if (good_stroke_neighbour(bnd)) ++score;
  }
  return score;
}
...
int BLOBNBOX::NoisyNeighbours() const {
  ...
  for (int dir = 0; dir < BND_COUNT; ++dir) {
    ...
    if (blob != nullptr && blob->region_type() == BRT_NOISE) ++count;
  }
  return count;
}
...
void BLOBNBOX::PlotNoiseBlobs(BLOBNBOX_LIST* list,
  ...
  for (it.mark_cycle_pt(); !it.cycled_list(); it.forward()) {
    ...
    if (blob->DeletableNoise()) blob->plot(win, body_colour, child_colour);
  }
}
...
ScrollView::Color BLOBNBOX::TextlineColor(BlobRegionType region_type,
  ...
  switch (region_type) {
    ...
    case BRT_VERT_TEXT:
      ...
      if (flow_type == BTFT_CHAIN) return ScrollView::LIME_GREEN;
      return ScrollView::YELLOW;
    case BRT_TEXT:
      if (flow_type == BTFT_STRONG_CHAIN) return ScrollView::BLUE;
      if (flow_type == BTFT_TEXT_ON_IMAGE) return ScrollView::LIGHT_BLUE;
      if (flow_type == BTFT_CHAIN) return ScrollView::MEDIUM_BLUE;
      if (flow_type == BTFT_LEADER) return ScrollView::WHEAT;
      if (flow_type == BTFT_NONTEXT) return ScrollView::PINK;
...
void BLOBNBOX::plot(ScrollView* window,                // window to draw in
  ...
  if (cblob_ptr != nullptr) cblob_ptr->plot(window, blob_colour, child_colour);
}
#endif
/**********************************************************************
 * find_cblob_limits
...
void TO_ROW::add_blob(  // constructor
  ...
  if (allowed > 0) {
    ...
    if (available > 0) {
      ...
      if (bottom < y_min) y_min -= (y_min - bottom) * allowed / available;
      if (top > y_max) y_max += (top - y_max) * allowed / available;
    }
  }
}
