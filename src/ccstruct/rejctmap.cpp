...
bool REJ::rej_between_mm_and_quality_accept() { return flag(R_BAD_QUALITY); }
...
bool REJMAP::recoverable_rejects() {  // Any non perm rejs?
  for (int i = 0; i < len; i++) {
    if (ptr[i].recoverable()) return true;
  }
  return false;
}
...
bool REJMAP::quality_recoverable_rejects() {  // Any potential rejs?
  for (int i = 0; i < len; i++) {
    if (ptr[i].accept_if_good_quality()) return true;
  }
  return false;
}
