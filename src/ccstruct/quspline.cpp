...
QSPLINE& QSPLINE::operator=(  // assignment
  ...
  if (xcoords != nullptr) free_mem(xcoords);
  if (quadratics != nullptr) free_mem(quadratics);
...
bool QSPLINE::overlap(  // test overlap
  ...
  return !(spline2->segments < 3 ||
           spline2->xcoords[1] >
               leftlimit + fraction * (rightlimit - leftlimit) ||
...
void QSPLINE::extrapolate(  // linear extrapolation
  ...
  if (xmax > xcoords[segments]) increment++;
  if (increment == 0) return;
  xstarts = (int*)alloc_mem((segments + 1 + increment) * sizeof(int));
  quads = (QUAD_COEFFS*)alloc_mem((segments + increment) * sizeof(QUAD_COEFFS));
  if (xmin < xcoords[0]) {
    xstarts[0] = xmin;
    quads[0].a = 0;
    quads[0].b = gradient;
