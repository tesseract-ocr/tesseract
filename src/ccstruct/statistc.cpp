...
int32_t choose_nth_item(int32_t index, float *array, int32_t count) {
  ...
  if (count == 2) {
    ...
    if (index < next_lesser)
      ...
      return choose_nth_item(index - prev_greater, array + prev_greater, count - prev_greater) +
             prev_greater;
  }
}
...
int32_t choose_nth_item(int32_t index, void *array, int32_t count, size_t size,
  ...
  equal_count = 1;
  for (next_sample = 1; next_sample < prev_greater;) {
    result = compar(static_cast<char *>(array) + size * next_sample,
  ...
  if (index < next_lesser)
    ...
    return choose_nth_item(index - prev_greater, static_cast<char *>(array) + size * prev_greater,
