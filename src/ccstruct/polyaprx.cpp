...
TESSLINE* ApproximateOutline(bool allow_detailed_fx, C_OUTLINE* c_outline) {
  ...
  if (edgepts != stack_edgepts) delete[] edgepts;
...
void fix2(         // polygonal approx
  ...
  do {
    ...
    if (((dir1 - dir2 + 1) & 7) < 3) {
      while (edgept->prev->flags[DIR] == edgept->next->flags[DIR]) {
        ...
        if (edgept->flags[DIR] == dir1) /*sum lengths */
          sum1 += edgept->flags[RUNLENGTH];
        else
          sum2 += edgept->flags[RUNLENGTH];
      }

      if (edgept == loopstart) stopped = 1; /*finished */
  ...
  fixed_count = 0;
  do {
    if (edgept->flags[FLAGS] & FIXED) fixed_count++;
    edgept = edgept->next;
  } while (edgept != start);
  while ((edgept->flags[FLAGS] & FIXED) == 0) edgept = edgept->next;
  edgefix0 = edgept;
  ...
  do {
    if (fixed_count <= 3) break;  // already too few
...
EDGEPT* poly2(       // second poly
  ...
  if (area < 1200) area = 1200; /*minimum value */
  ...
  if (loopstart) {
    do {
      ...
      edgesum = 0;
      do {
        if (edgept->flags[FLAGS] & FIXED) edgesum++;
        edgept = edgept->next;
      }
      // count fixed pts
      while (edgept != loopstart);
      if (edgesum < 3) area /= 2;  // must have 3 pts
    } while (edgesum < 3);
    do {
      linestart = edgept;
      do {
...
void cutline(              // recursive refine
  ...
  if (edge->next == last) return; /*simple line */
  ...
  do {
    ...
    if (poly_debug) tprintf("Cutline:Final perp=%d\n", perp);
    if (perp > maxperp) {
      maxperp = perp;
  ...
  if (squaresum < 256 * INT16_MAX) /*mean squared perp */
