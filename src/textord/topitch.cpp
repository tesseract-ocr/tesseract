...
void compute_fixed_pitch(ICOORD page_tr,              // top right
  ...
  if (textord_show_initial_words && testing_on) {
    if (to_win == nullptr) create_to_win(page_tr);
  }
#endif
...
void compute_block_pitch(TO_BLOCK* block,      // input list
  ...
  block->min_space =
      (int32_t)floor(block->xheight * textord_words_default_minspace);
  block->max_nonspace =
      (int32_t)ceil(block->xheight * textord_words_default_nonspace);
...
bool try_doc_fixed(              // determine pitch
  ...
  for (block_it.mark_cycle_pt(); !block_it.cycled_list(); block_it.forward()) {
    ...
    for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
      row = row_it.data();
      total_row_count++;
      if (row->fixed_pitch > 0) pitches.add((int32_t)(row->fixed_pitch), 1);
      // find median
      row_y = row->baseline.y(master_x);
      row_left =
          (int16_t)(row->projection_left - shift_factor * (master_y - row_y));
      row_right =
          (int16_t)(row->projection_right - shift_factor * (master_y - row_y));
      if (row_left < projection_left) projection_left = row_left;
      if (row_right > projection_right) projection_right = row_right;
    }
  }
  if (pitches.get_total() == 0) return false;
...
void count_block_votes(  // find line stats
    ...
    int32_t& def_prop, int32_t& maybe_fixed, int32_t& maybe_prop,
    int32_t& corr_fixed, int32_t& corr_prop, int32_t& dunno) {
  TO_ROW* row;  // current row
  TO_ROW_IT row_it = block->get_rows();
...
bool row_pitch_stats(  // find line stats
  ...
  if (gap_index == 0) {
    ...
    while (gap_index < cluster_count && gaps[gap_index] < upper) gap_index++;
...
bool find_row_pitch(      // find lines
  ...
  if (non_space > initial_pitch) non_space = initial_pitch;
  ...
  if (pitch_iqr > maxwidth && dm_pitch_iqr > maxwidth) {
    row->pitch_decision = PITCH_DUNNO;
    if (textord_debug_pitch_metric) tprintf("\n");
...
bool fixed_pitch_row(TO_ROW* row,  // row to do
  ...
  non_space = row->fp_nonsp;
  if (non_space > row->fixed_pitch) non_space = row->fixed_pitch;
  ...
  do {
    ...
    if (!blob->joined_to_prev()) {
      ...
      if ((blob_box.left() - joined_box.right() < dm_gap &&
        ...
        if (split_outsize) {
          width_units = (int32_t)floor((float)blob_width / initial_pitch + 0.5);
...
float tune_row_pitch(            // find fp cells
  ...
  for (pitch_delta = 1; pitch_delta <= textord_pitch_range; pitch_delta++) {
    ...
    if (pitch_sd < best_sd) {
      ...
      test_cells.clear();
    if (pitch_sd > initial_sd) break;  // getting worse
  }
  for (pitch_delta = 1; pitch_delta <= textord_pitch_range; pitch_delta++) {
    pitch_sd = compute_pitch_sd(
        row, projection, projection_left, projection_right, space_size,
...
float compute_pitch_sd(         // find fp cells
  ...
  spsum = 0;
  prev_right = -1;
  if (blob_it.empty()) return space_size * 10;
#ifndef GRAPHICS_DISABLED
  ...
  do {
    for (; blob_count > 0; blob_count--) box_next(&start_it);
    do {
      prev_box = blob_box;
      blob_count++;
    ...
    if (testing_on) {
      ...
      for (seg_it.mark_cycle_pt(); !seg_it.cycled_list(); seg_it.forward()) {
        if (seg_it.data()->faked) tprintf("(F)");
...
float compute_pitch_sd2(        // find fp cells
  ...
  plot_it = blob_it;
  word_sync = check_pitch_sync2(&blob_it, blob_count, (int16_t)initial_pitch, 2,
                                projection, projection_left, projection_right,
  ...
  for (seg_it.mark_cycle_pt(); !seg_it.cycled_list(); seg_it.forward()) {
    ...
    cell = new ICOORDELT(segpos, 0);
    cell_it.add_after_then_move(cell);
    if (seg_it.at_last()) mid_cuts = seg_it.data()->cheap_cuts();
  }
  seg_list.clear();
...
void print_pitch_sd(          // find fp cells
  ...
  do {
    ...
    word_sync = check_pitch_sync2(
        &start_it, blob_count, (int16_t)initial_pitch, 2, projection,
  ...
  blob_it = row_start;
  word_sync = check_pitch_sync2(
      &blob_it, total_blob_count, (int16_t)initial_pitch, 2, projection,
      projection_left, projection_right,
      row->xheight * textord_projection_scale, occupation, &seg_list, 0, 0);
  if (occupation > 1) word_sync /= occupation;
