...
ROW* fixed_pitch_words(  // find lines
  ...
  if (box_it.empty()) return nullptr;  // empty row
  ...
  for (; !cell_it.cycled_list(); cell_it.forward()) {
    ...
    if (prev_chop_coord < chop_coord) prev_chop_coord = chop_coord;
  }
  if (!cblob_it.empty()) {
    word = new WERD(&cblobs, blanks, nullptr);
    word->set_flag(W_DONT_CHOP, TRUE);
    word_it.add_after_then_move(word);
    if (bol) word->set_flag(W_BOL, TRUE);
  }
  ...
  if (prev_chop_coord > prev_x) prev_x = prev_chop_coord;
  xstarts[1] = prev_x + 1;
...
bool fixed_chop_coutline(             // chop the outline
  ...
  if (left_edge >= chop_coord - pitch_error) return false;  // not worth it
  ...
  do {
    do {
      ...
      tail_index++;
      if (tail_index == length) tail_index = 0;
...
void save_chop_cfragment(       // chop the outline
  ...
  if (stepcount < 0) stepcount += srcline->pathlength();
  jump = tail_pos.y() - head_pos.y();
  if (jump < 0) jump = -jump;
  if (jump == stepcount) return;  // its a nop
  tail_y = tail_pos.y();
  head =
...
C_OUTLINE_FRAG::C_OUTLINE_FRAG(  // record fragment
  ...
  if (stepcount < 0) stepcount += outline->pathlength();
  ...
  if (end_index > start_index) {
    ...
    int i = start_index;
    for (; i < len; ++i) steps[i - start_index] = outline->step_dir(i);
    if (end_index > 0)
    ...
    if (top_frag->other_end != bottom_frag) {
      ...
      if (outline != nullptr) {
        ...
        for (child_it.mark_cycle_pt(); !child_it.cycled_list();
             child_it.forward()) {
          child = child_it.data();
          if (*child < *outline) olchild_it.add_to_end(child_it.extract());
        }
