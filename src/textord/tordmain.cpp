...
void Textord::filter_blobs(ICOORD page_tr,         // top right
  ...
  if (to_win != nullptr) to_win->Clear();
    ...
    if (clean_noise) {
      ...
      for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
        ...
        if ((textord_noise_rejrows && !row->word_list()->empty() &&
          ...
          if (textord_noise_rejwords) clean_noise_from_words(row_it.data());
...
bool Textord::clean_noise_from_row(  // remove empties
  ...
  testing_on =
      textord_test_y > row->base_line(textord_test_x) && textord_show_blobs &&
...
void tweak_row_baseline(ROW* row, double blshift_maxshift,
  ...
  xstarts = (int32_t*)alloc_mem((blob_count + row->baseline.segments + 1) *
                                sizeof(int32_t));
