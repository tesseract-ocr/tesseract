...
void restore_underlined_blobs(  // get chop points
  ...
  if (block->get_rows()->empty()) return;  // Don't crash if there are no rows.
  for (under_it.mark_cycle_pt(); !under_it.cycled_list(); under_it.forward()) {
    ...
    if (row == nullptr) return;  // Don't crash if there is no row.
...
TO_ROW* most_overlapping_row(  // find best row
  ...
  if (row_it.empty()) return nullptr;
  row = row_it.data();
...
void vertical_cunderline_projection(  // project outlines
  ...
  for (stepindex = 0; stepindex < length; stepindex++) {
    ...
    if (step.x() > 0) {
      lower_y = (int16_t)floor(baseline->y(pos.x()) + baseline_offset + 0.5);
      upper_y = (int16_t)floor(baseline->y(pos.x()) + baseline_offset +
                               xheight + 0.5);
      if (pos.y() >= lower_y) {
      ...
      lower_y =
          (int16_t)floor(baseline->y(pos.x() - 1) + baseline_offset + 0.5);
      upper_y = (int16_t)floor(baseline->y(pos.x() - 1) + baseline_offset +
                               xheight + 0.5);
      if (pos.y() >= lower_y) {
