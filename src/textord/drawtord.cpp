...
void draw_occupation(  // draw projection
    ...
    int32_t max_y,
    int32_t occupation[],  // projection counts
...
void plot_word_decisions(  // draw words
  ...
  for (blob_it.mark_cycle_pt(); !blob_it.cycled_list(); blob_it.forward()) {
    ...
    if (!blob->joined_to_prev()) prev_x = blob_box.right();
    if (blob->cblob() != nullptr) blob->cblob()->plot(win, colour, colour);
    if (!blob->joined_to_prev() && blob->cblob() != nullptr) blob_count++;
  }
...
void plot_fp_cells(            // draw words
  ...
  for (; blob_count > 0; blob_count--) word_box += box_next(blob_it);
