...
void FPCUTPT::setup(       // constructor
  ...
  if (x == array_origin) {
    ...
    for (ind = 0; ind <= half_pitch; ind++) {
      fwd_balance >>= 1;
      if (projection->pile_count(ind) > zero_count) fwd_balance |= lead_flag;
    }
  } else {
    back_balance = cutpts[x - 1 - array_origin].back_balance << 1;
    back_balance &= lead_flag + lead_flag - 1;
    if (projection->pile_count(x) > zero_count) back_balance |= 1;
...
double check_pitch_sync2(     // find segmentation
  ...
  zero_count = 0;
  if (pitch < 3) pitch = 3;  // nothing ludicrous
  if ((pitch - 3) / 2 < pitch_error) pitch_error = (pitch - 3) / 2;
  this_it = *blob_it;
  ...
  while (x < right_edge - pitch_error) {
    ...
    if (x <= this_box.left())
      ...
      if (this_box.right() - x < offset) offset = this_box.right() - x;
  ...
  do {
    for (x = best_end->position() - pitch + pitch_error;
         ...
         x++)
      ;
    if (x < best_end->position() - pitch_error) occupation_count++;
    // copy it
...
double check_pitch_sync3(     // find segmentation
  ...
  while (x < right_edge - pitch_error) {
    ...
    minindex++;
    if (minindex > pitch_error * 2) minindex = 0;
    faking = false;
    mid_cut = false;
    offset = 0;
    ...
    if (offset > pitch_error) {
      if (x - prev_zero > zero_offset && next_zero - x > zero_offset) {
        for (offset = 0; offset <= pitch_error; offset++) {
          ...
          if (test_index > pitch_error * 2) test_index -= pitch_error * 2 + 1;
          if (mins[test_index]) break;
          test_index = minindex + pitch_error - offset;
          if (test_index > pitch_error * 2) test_index -= pitch_error * 2 + 1;
          if (mins[test_index]) break;
        }
      ...
      if (offset > pitch_error) {
        ...
        if (projection_offset > offset) offset = projection_offset;
        mid_cut = true;
      }
    }
