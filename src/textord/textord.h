...
class Textord {
 ...
 private:
  ...
  void row_spacing_stats(TO_ROW* row, GAPMAP* gapmap, int16_t block_idx,
                         int16_t row_idx,
                         // estimate for block
                         int16_t block_space_gap_width,
                         // estimate for block
  ...
  void old_to_method(TO_ROW* row, STATS* all_gap_stats, STATS* space_gap_stats,
                     ...
                     // estimate for block
                     int16_t block_non_space_gap_width);
  bool isolated_row_stats(TO_ROW* row, GAPMAP* gapmap, STATS* all_gap_stats,
                          bool suspected_table, int16_t block_idx,
                          int16_t row_idx);
  ...
  bool make_a_word_break(TO_ROW* row,    // row being made
                         ...
                         int16_t prev_gap, TBOX prev_blob_box,
                         int16_t real_current_gap,
                         int16_t within_xht_current_gap, TBOX next_blob_box,
                         int16_t next_gap, uint8_t& blanks, bool& fuzzy_sp,
                         bool& fuzzy_non, bool& prev_gap_was_a_space,
  ...
  bool ignore_big_gap(TO_ROW* row, int32_t row_length, GAPMAP* gapmap,
                      int16_t left, int16_t right);
  // get bounding box
  TBOX reduced_box_next(TO_ROW* row,     // current row
                        BLOBNBOX_IT* it  // iterator to blobds
  );
  TBOX reduced_box_for_blob(BLOBNBOX* blob, TO_ROW* row,
