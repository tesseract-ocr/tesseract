...
void Textord::find_textlines(TO_BLOCK* block,    // block row is in
  ...
  if (jumplimit < MINASCRISE) jumplimit = MINASCRISE;
...
int get_blob_coords(     // get boxes
  ...
  if (blob_it.empty()) return 0;  // none
  maxlosscount = 0;
  losscount = 0;
  ...
  if (heightstat.get_total() > 1) /*guess x-height */
...
void make_first_baseline(  // initial approximation
  ...
  if (spline == nullptr       /*no given spline */
    ...
    if (textord_oldbl_paradef) return;  // use default
      ...
      for (blobindex = 2; blobindex < blobcount; blobindex++) {
        ...
        if (ABS(thisy - prevy) < jumplimit && ABS(thisy - nexty) < jumplimit) {
          ...
          if (ycount == 1) {
            ...
            if (y3 > maxmax) maxmax = y3; /*biggest max */
            if (y3 < minmin) minmin = y3; /*smallest min */
          }
          /*possible turning pt */
  ...
  for (blobindex = 0; blobindex < blobcount; blobindex++) {
    if (partids[blobindex] != prevpart) {
      ...
      if (prevpart != biggestpart && runlength > MAXBADRUN) {
        ...
        m = stats.get_b();
        c = stats.get_c();
        if (textord_oldbl_debug) tprintf("Fitted line y=%g x + %g\n", m, c);
        found_one = false;
        close_one = false;
        for (test_blob = 1; !found_one && (startx - test_blob >= 0 ||
          ...
          if (startx - test_blob >= 0 &&
            ...
            if (diff < jumplimit && -diff < jumplimit) close_one = true;
          }
...
int segment_spline(             // make xstarts
  ...
  if (pointcount > 3) {
    ...
    if (ycoords[ptindex] < ycoords[lastmax] - TURNLIMIT &&
      ...
      if (turncount < SPLINESIZE - 1) /*2 more turns */
        turnpoints[turncount++] = lastmax;
      if (turncount < SPLINESIZE - 1) turnpoints[turncount++] = ptindex;
...
bool split_stepped_spline(  // make xstarts
  ...
  for (segment = 1; segment < segments - 1; segment++) {
    ...
    if (step < 0) step = -step;
    if (step > jumplimit) {
      while (xcoords[startindex] < xstarts[segment - 1]) startindex++;
      centreindex = startindex;
      while (xcoords[centreindex] < xstarts[segment]) centreindex++;
      endindex = centreindex;
      while (xcoords[endindex] < xstarts[segment + 1]) endindex++;
...
void find_lesser_parts(  // get descenders
  ...
  for (runlength = 0, blobindex = 0; blobindex < blobcount; blobindex++) {
    ...
    if (part_id != bestpart) {
      ...
      if (runlength > biggestrun) biggestrun = runlength;
      partsteps[part_id] +=
...
void old_first_xheight(  // the wiseowl way
  ...
  if (blobcount > 1) {
    ...
    if (heightstat.get_total() > 3) {
      ...
      if (lineheight <= 0) lineheight = (int)heightstat.ile(0.5);
    } else
  ...
  if (row->xheight == 0) row->xheight = -1.0f;
}
...
void make_first_xheight(  // find xheight
    ...
    if (blobcoords[blobindex].height() > init_lineheight * kMinHeight) {
      if (height > lineheight * oldbl_xhfract && height > textord_min_xheight) {
        ...
        if (height < HEIGHTBUCKETS) {
          if (xcenter > rights[height]) rights[height] = xcenter;
  ...
  if (textord_oldbl_debug) tprintf("Output xheight=%g\n", row->xheight);
  if (row->xheight < 0 && textord_oldbl_debug)
    tprintf("warning: Row Line height < 0; %4.2f\n", row->xheight);

  if (sign_bit < 0) row->xheight = -row->xheight;
}
...
void find_top_modes(  // get modes
    ...
    int modelist[],
    int modenum  // no of modes to get
) {
  int mode_count;
  int last_i = 0;
  ...
  for (mode_count = 0; mode_count < modenum; mode_count++) {
    ...
    total_max += last_max;
    if (last_max <= total_max / mode_factor) mode = 0;
