...
float make_single_row(ICOORD page_tr, bool allow_sub_blobs, TO_BLOCK* block,
  ...
  if (block->blobs.singleton() && allow_sub_blobs) {
    ...
    if (size > block->line_size) block->line_size = size;
...
#endif
  // guess skew
  assign_blobs_to_rows(block, nullptr, 0, TRUE, TRUE,
...
void compute_page_skew(     // get average gradient
  ...
  row_index = choose_nth_item((int32_t)(row_count * textord_skew_ile), errors,
                              row_count);
...
void vigorous_noise_removal(TO_BLOCK* block) {
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    for (b_it.mark_cycle_pt(); !b_it.cycled_list(); b_it.forward()) {
      ...
      if (height >= kMinSize) hstats.add(blob->bounding_box().height(), 1);
    }
    ...
    for (b_it.mark_cycle_pt(); !b_it.cycled_list(); b_it.forward()) {
      ...
      if (box.height() < kNoiseSize * xheight) {
        ...
        if (prev != nullptr) {
          if (dot_of_i(blob, prev, row)) continue;  // Looks OK.
        }
        if (!b_it.at_last()) {
          BLOBNBOX* next = b_it.data_relative(1);
          if (dot_of_i(blob, next, row)) continue;  // Looks OK.
        }
...
void delete_non_dropout_rows(  // find lines
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    if (line_index <= min_y) min_y = line_index - 1;
    if (line_index >= max_y) max_y = line_index + 1;
  }
...
void compute_row_stats(  // find lines
  ...
  if (rows == nullptr) MEMORY_OUT.error("compute_row_stats", ABORT, nullptr);
  rowcount = 0;
  prev_row = nullptr;
  ...
  if (rowcount > 0) {
    ...
    if (testing_on) tprintf(" row based=%g(%g)", rows[row_index]->spacing, iqr);
    if (rowcount > 2 &&
...
int compute_xheight_from_modes(STATS* heights, STATS* floating_heights,
  ...
  int mode_count = compute_height_modes(heights, min_height, max_height, modes,
                                        MAX_HEIGHT_MODES);
  if (cap_only && mode_count > 1) mode_count = 1;
  int x;
  if (textord_debug_xheights) {
  ...
  for (x = 0; x < mode_count - 1; x++) {
    if (modes[x] != prev_size + 1) in_best_pile = FALSE;  // had empty height
    int modes_x_count =
...
void assign_blobs_to_rows(  // find lines
  ...
  ycoord = (block->block->pdblk.bounding_box().bottom() +
            block->block->pdblk.bounding_box().top()) /
           2.0f;
  if (gradient != nullptr) g_length = sqrt(1 + *gradient * *gradient);
#ifndef GRAPHICS_DISABLED
  if (drawing_skew)
 ...
 */
OVERLAP_STATE
most_overlapping_row(   // find best row
  ...
  row = row_it->data();
  bestover = top - bottom;
  if (top > row->max_y()) bestover -= top - row->max_y();
  if (bottom < row->min_y())
    // compute overlap
  ...
  do {
    if (!row_it->at_last()) {
      ...
      if (test_row->min_y() <= top && test_row->max_y() >= bottom) {
        ...
        overlap = top - bottom;
        if (top > test_row->max_y()) overlap -= top - test_row->max_y();
        if (bottom < test_row->min_y()) overlap -= test_row->min_y() - bottom;
    ...
    do {
      ...
      if (bblob->flow() == BTFT_LEADER && !bblob->joined_to_prev() &&
        ...
        for (test_it.forward(); !test_it.at_first();) {
          bblob = test_it.data();
          if (bblob->flow() != BTFT_LEADER) break;
          test_it.forward();
          bblob = test_it.data();
