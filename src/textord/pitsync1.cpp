...
FPSEGPT::FPSEGPT(            // constructor
  ...
  for (pred_it.mark_cycle_pt(); !pred_it.cycled_list(); pred_it.forward()) {
    segpt = pred_it.data();
    if (segpt->fake_count < best_fake) best_fake = segpt->fake_count;
    dist = x - segpt->xpos;
  ...
  if (fake_count > best_fake + 1) pred = nullptr;  // fail it
}
...
double check_pitch_sync(    // find segmentation
  ...
  do {
    ...
    if (segpts->empty()) {
      if (best_end != nullptr) break;  // already found one
...
void make_illegal_segment(    // find segmentation
  ...
  for (x = min_x; x <= max_x; x++) {
    ...
    if (blob_box.right() - x < offset) offset = blob_box.right() - x;
    segpt = new FPSEGPT(x, FALSE, offset, region_index, pitch, pitch_error,
                        prev_list);
    if (segpt->previous() != nullptr) {
