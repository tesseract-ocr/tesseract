...
void set_row_spaces(  // find space sizes
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    if (row->fixed_pitch == 0) {
      row->min_space =
          (int32_t)ceil(row->pr_space - (row->pr_space - row->pr_nonsp) *
                                            textord_words_definite_spread);
      row->max_nonspace =
          (int32_t)floor(row->pr_nonsp + (row->pr_space - row->pr_nonsp) *
                                             textord_words_definite_spread);
...
int32_t row_words(     // compute space size
  ...
  for (blob_it.mark_cycle_pt(); !blob_it.cycled_list(); blob_it.forward()) {
    ...
    if (blob_box.contains(testpt)) testing_row = true;
...
ROW* make_rep_words(  // make a row
  ...
  if (word_it.empty()) return nullptr;
  word_box = word_it.data()->bounding_box();
  for (word_it.mark_cycle_pt(); !word_it.cycled_list(); word_it.forward())
    word_box += word_it.data()->bounding_box();
  row->xheight = block->xheight;
  real_row =
      new ROW(row, (int16_t)block->kern_size, (int16_t)block->space_size);
...
WERD* make_real_word(BLOBNBOX_IT* box_it,  // iterator
  ...
  if (box_it->at_first()) word->set_flag(W_EOL, true);  // at end of line
