...
void BBGrid<BBC, BBC_CLIST, BBC_C_IT>::InsertBBox(bool h_spread, bool v_spread,
  ...
  if (!h_spread) end_x = start_x;
  if (!v_spread) end_y = start_y;
...
BBC* GridSearch<BBC, BBC_CLIST, BBC_C_IT>::NextRadSearch() {
  do {
    while (it_.cycled_list()) {
      ...
      if (rad_index_ >= radius_) {
        ...
        if (rad_dir_ >= 4) {
          ++radius_;
          if (radius_ > max_radius_) return CommonEnd();
          rad_dir_ = 0;
        }
      }
  ...
  if (unique_mode_) returns_.insert(previous_return_);
  return previous_return_;
}
    ...
    while (it_.cycled_list()) {
      ...
      if (rad_index_ > radius_) {
        if (right_to_left)
          ...
          ++x_;
        rad_index_ = 0;
        if (x_ < 0 || x_ >= grid_->gridwidth_) return CommonEnd();
      }
      y_ = y_origin_ - rad_index_;
      if (y_ >= 0 && y_ < grid_->gridheight_) SetIterator();
    }
    CommonNext();
  } while (unique_mode_ && returns_.find(previous_return_) != returns_.end());
  if (unique_mode_) returns_.insert(previous_return_);
    ...
    while (it_.cycled_list()) {
      ...
      if (rad_index_ > radius_) {
        if (top_to_bottom)
          ...
          ++y_;
        rad_index_ = 0;
        if (y_ < 0 || y_ >= grid_->gridheight_) return CommonEnd();
      }
      x_ = x_origin_ + rad_index_;
      if (x_ >= 0 && x_ < grid_->gridwidth_) SetIterator();
    }
    CommonNext();
  } while (unique_mode_ && returns_.find(previous_return_) != returns_.end());
  if (unique_mode_) returns_.insert(previous_return_);
  ...
  do {
    while (it_.cycled_list()) {
      ...
      if (x_ > max_radius_) {
        --y_;
        x_ = x_origin_;
        if (y_ < y_origin_) return CommonEnd();
      }
      SetIterator();
    }
    CommonNext();
