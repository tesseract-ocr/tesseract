...
void Textord::block_spacing_stats(
    TO_BLOCK* block, GAPMAP* gapmap, bool& old_text_ord_proportional,
  ...
  for (row_it.mark_cycle_pt(); !row_it.cycled_list(); row_it.forward()) {
    ...
    if (!row->blob_list()->empty() &&
      ...
      if (blob_box.width() < minwidth) minwidth = blob_box.width();
      ...
      while (!blob_it.cycled_list()) {
        ...
        if (!ignore_big_gap(row, row_length, gapmap, left, right)) {
          ...
          centre_to_centre =
              (right + blob_box.right() - (prev_blob_box.left() + left)) / 2;
          // DEBUG
...
void Textord::row_spacing_stats(
    TO_ROW* row, GAPMAP* gapmap, int16_t block_idx, int16_t row_idx,
  ...
  if (!row->blob_list()->empty()) {
    if (tosp_threshold_bias1 > 0)
      real_space_threshold =
          ...
          int16_t(
              floor(0.5 + tosp_threshold_bias1 * (block_space_gap_width -
                                                  block_non_space_gap_width)));
    else
  ...
  if ((cert_space_gap_stats.get_total() >=
    ...
    if (!tosp_recovery_isolated_row_stats ||
      ...
      if (tosp_row_use_cert_spaces1 && good_block_space_estimate) {
        ...
        row->space_threshold = int32_t(floor(
            (row->space_size + row->kern_size) / tosp_old_sp_kn_th_factor));
      } else
  ...
  if (tosp_sanity_method == 0) {
    if (suspected_table &&
      ...
      row->space_threshold = (int32_t)(tosp_table_kn_sp_ratio * row->kern_size);
    ...
    if ((row->space_size <
      ...
      if (good_block_space_estimate &&
        ...
        sane_space = std::max(static_cast<float>(tosp_min_sane_kn_sp) *
    ...
    sane_threshold = int32_t(
        floor(tosp_max_sane_kn_thresh * std::max(row->kern_size, 2.5f)));
  ...
  if ((tosp_table_fuzzy_kn_sp_ratio > 0) &&
    ...
    row->min_space =
        std::max(row->min_space,
                 (int32_t)ceil(tosp_table_fuzzy_kn_sp_ratio * row->kern_size));

  if ((tosp_fuzzy_kn_fraction > 0) && (row->kern_size < row->space_threshold)) {
    row->max_nonspace = (int32_t)floor(
        0.5 + row->kern_size +
 ...
 * Set values for min_space, max_non_space based on row stats only
 *************************************************************************/
bool Textord::isolated_row_stats(TO_ROW* row, GAPMAP* gapmap,
                                 STATS* all_gap_stats, bool suspected_table,
  ...
  crude_threshold_estimate = std::max(tosp_init_guess_kn_mult * kern_estimate,
                                      tosp_init_guess_xht_mult * row->xheight);
  small_gaps_count =
      stats_count_under(all_gap_stats, (int16_t)ceil(crude_threshold_estimate));
  ...
  while (!blob_it.cycled_list()) {
    ...
    if (gap_width < crude_threshold_estimate) small_gap_stats.add(gap_width, 1);
...
int16_t Textord::stats_count_under(STATS* stats, int16_t threshold) {
  ...
  for (index = 0; index < threshold; index++) total += stats->pile_count(index);
  return total;
}
...
void Textord::improve_row_threshold(TO_ROW* row, STATS* all_gap_stats) {
  ...
  if (tosp_debug_level > 10) tprintf("Improve row threshold 0");
  ...
  */
  ...
  for (index = int16_t(ceil(kn)); index < int16_t(floor(sp)); index++) {
    if (all_gap_stats->pile_count(index) == 0) {
      if (zero_width == 0) zero_start = index;
      zero_width++;
    } else {
  ...
  if ((zero_width < reqd_zero_width) ||
    ...
    return;
  if (tosp_debug_level > 10) tprintf(" 2");
      ...
      if ((prev_blob_box.width() > 0) && (next_blob_box.width() > 0) &&
        ...
        */
        ...
        mark_gap(blob_box, 8, prev_gap, prev_blob_box.width(), current_gap,
                 ...
                 (current_gap >=
                  tosp_kern_gap_factor3 * std::max(prev_gap, next_gap)) &&
...
void Textord::mark_gap(TBOX blob,     // blob following gap
                       ...
                       int16_t prev_gap, int16_t prev_blob_width,
                       int16_t current_gap, int16_t next_blob_width,
                       int16_t next_gap) {
...
bool Textord::ignore_big_gap(TO_ROW* row, int32_t row_length, GAPMAP* gapmap,
  ...
  if (gap > tosp_ignore_very_big_gaps * row->xheight) return true;
  if (tosp_ignore_big_gaps == 0) {
