...
void ParseArguments(int* argc, char*** argv) {
  ...
  Config.MinSamples = std::max(
      0.0, std::min(1.0, double(FLAGS_clusterconfig_min_samples_fraction)));
  Config.MaxIllegal =
...
FindList(LIST List, char* Label) {
  ...
  iterate(List) {
    ...
    if (strcmp(LabeledList->Label, Label) == 0) return (LabeledList);
  }
  return (nullptr);
...
void ReadTrainingSamples(const FEATURE_DEFS_STRUCT& feature_defs,
  ...
  while (fgets(buffer, 2048, file) != nullptr) {
    ...
    for (size_t i = 0; i < char_desc->NumFeatureSets; i++) {
      if (feature_type != i) FreeFeatureSet(char_desc->FeatureSets[i]);
    }
    free(char_desc);
  }
}  // ReadTrainingSamples
...
CLUSTERER* SetUpForClustering(const FEATURE_DEFS_STRUCT& FeatureDefs,
  ...
  iterate(FeatureList) {
    ...
    for (i = 0; i < FeatureSet->MaxNumFeatures; i++) {
      ...
      for (j = 0; j < N; j++) Sample[j] = FeatureSet->Features[i]->Params[j];
...
void MergeInsignificantProtos(LIST ProtoList, const char* label,
  ...
  iterate(pProtoList) {
    ...
    if (Prototype->Significant || Prototype->Merged) continue;
...

{
  ...
  iterate(pProtoList) {
    ...
    if ((Proto->Significant && KeepSigProtos) ||
      ...
      for (i = 0; i < N; i++) NewProto->Mean[i] = Proto->Mean[i];
...
FindClass(LIST List, const char* Label) {
  ...
  iterate(List) {
    ...
    if (strcmp(MergeClass->Label, Label) == 0) return (MergeClass);
  }
  return (nullptr);
...
CLASS_STRUCT* SetUpForFloat2Int(const UNICHARSET& unicharset,
  ...
  iterate(LabeledClassList) {
    ...
    for (i = 0; i < NumConfigs; i++) {
      ...
      for (j = 0; j < NumWords; j++) NewConfig[j] = OldConfig[j];
