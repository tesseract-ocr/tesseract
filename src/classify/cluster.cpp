...
PROTOTYPE* MakeDegenerateProto(uint16_t N, CLUSTER* Cluster,
...
STATISTICS* ComputeStatistics(int16_t N, PARAM_DESC ParamDesc[],
                              CLUSTER* Cluster);

PROTOTYPE* NewSphericalProto(uint16_t N, CLUSTER* Cluster,
                             STATISTICS* Statistics);

PROTOTYPE* NewEllipticalProto(int16_t N, CLUSTER* Cluster,
                              STATISTICS* Statistics);
...
bool Independent(PARAM_DESC* ParamDesc, int16_t N, FLOAT32* CoVariance,
                 FLOAT32 Independence);

BUCKETS* GetBuckets(CLUSTERER* clusterer, DISTRIBUTION Distribution,
                    uint32_t SampleCount, FLOAT64 Confidence);

BUCKETS* MakeBuckets(DISTRIBUTION Distribution, uint32_t SampleCount,
                     FLOAT64 Confidence);
...
FLOAT64
NormalDensity(int32_t x);

FLOAT64
UniformDensity(int32_t x);

FLOAT64
Integral(FLOAT64 f1, FLOAT64 f2, FLOAT64 Dx);

void FillBuckets(BUCKETS* Buckets, CLUSTER* Cluster, uint16_t Dim,
                 PARAM_DESC* ParamDesc, FLOAT32 Mean, FLOAT32 StdDev);

uint16_t NormalBucket(PARAM_DESC* ParamDesc, FLOAT32 x, FLOAT32 Mean,
                      FLOAT32 StdDev);

uint16_t UniformBucket(PARAM_DESC* ParamDesc, FLOAT32 x, FLOAT32 Mean,
                       FLOAT32 StdDev);
...
FLOAT64
ChiArea(CHISTRUCT* ChiParams, FLOAT64 x);

bool MultipleCharSamples(CLUSTERER* Clusterer, CLUSTER* Cluster,
                         FLOAT32 MaxIllegal);
...
SAMPLE* MakeSample(CLUSTERER* Clusterer, const FLOAT32* Feature,
  ...
  for (i = 0; i < Clusterer->SampleSize; i++) Sample->Mean[i] = Feature[i];
  ...
  if (CharID >= Clusterer->NumChar) Clusterer->NumChar = CharID + 1;
...
LIST ClusterSamples(CLUSTERER* Clusterer, CLUSTERCONFIG* Config) {
  ...
  if (Clusterer->Root == nullptr) CreateClusterTree(Clusterer);
...
void FreeClusterer(CLUSTERER* Clusterer) {
  if (Clusterer != nullptr) {
    ...
    if (Clusterer->KDTree != nullptr) FreeKDTree(Clusterer->KDTree);
    if (Clusterer->Root != nullptr) FreeCluster(Clusterer->Root);
...
void FreePrototype(void* arg) {  // PROTOTYPE     *Prototype)
  ...
  if (Prototype->Cluster != nullptr) Prototype->Cluster->Prototype = FALSE;
...
CLUSTER* NextSample(LIST* SearchState) {
  ...
  if (*SearchState == NIL_LIST) return (nullptr);
  Cluster = (CLUSTER*)first_node(*SearchState);
  *SearchState = pop(*SearchState);
  while (TRUE) {
    if (Cluster->Left == nullptr) return (Cluster);
 ...
 */
FLOAT32
Mean(PROTOTYPE* Proto, uint16_t Dimension) {
 ...
 */
FLOAT32
StandardDeviation(PROTOTYPE* Proto, uint16_t Dimension) {
  switch (Proto->Style) {
    case spherical:
    ...
    if (ParamDesc->Circular) {
      ...
      if ((*m2 - *m1) > ParamDesc->HalfRange) {
        *m = (n1 * *m1 + n2 * (*m2 - ParamDesc->Range)) / n;
        if (*m < ParamDesc->Min) *m += ParamDesc->Range;
...
PROTOTYPE* MakeDegenerateProto(  // this was MinSample
    uint16_t N, CLUSTER* Cluster, STATISTICS* Statistics, PROTOSTYLE Style,
    int32_t MinSamples) {
  PROTOTYPE* Proto = nullptr;

  if (MinSamples < MINSAMPLESNEEDED) MinSamples = MINSAMPLESNEEDED;
...
PROTOTYPE* TestEllipticalProto(CLUSTERER* Clusterer, CLUSTERCONFIG* Config,
  ...
  if (TotalDims < N + 1 || TotalDims < 2) return nullptr;
  ...
  int Fx = EssentialN;
  if (Fx > FTABLE_X) Fx = FTABLE_X;
  --Fx;
  int Fy = TotalDims - EssentialN - 1;
  if (Fy > FTABLE_Y) Fy = FTABLE_Y;
  --Fy;
...
PROTOTYPE* MakeMixedProto(CLUSTERER* Clusterer, CLUSTER* Cluster,
  ...
  for (i = 0; i < Clusterer->SampleSize; i++) {
    ...
    if (DistributionOK(NormalBuckets)) continue;
    ...
    if (DistributionOK(RandomBuckets)) continue;
    ...
    FillBuckets(UniformBuckets, Cluster, i, &(Clusterer->ParamDesc[i]),
                Proto->Mean[i], Proto->Variance.Elliptical[i]);
    if (DistributionOK(UniformBuckets)) continue;
    break;
  }
...
STATISTICS* ComputeStatistics(int16_t N, PARAM_DESC ParamDesc[],
  ...
  for (i = 0; i < N; i++) {
    ...
    for (j = 0; j < N; j++, CoVariance++) *CoVariance = 0;
  }
  ...
  while ((Sample = NextSample(&SearchState)) != nullptr) {
    for (i = 0; i < N; i++) {
      ...
      if (Distance[i] < Statistics->Min[i]) Statistics->Min[i] = Distance[i];
      if (Distance[i] > Statistics->Max[i]) Statistics->Max[i] = Distance[i];
    }
    CoVariance = Statistics->CoVariance;
    for (i = 0; i < N; i++)
  ...
  for (i = 0; i < N; i++)
    for (j = 0; j < N; j++, CoVariance++) {
      *CoVariance /= SampleCountAdjustedForBias;
      if (j == i) {
        if (*CoVariance < MINVARIANCE) *CoVariance = MINVARIANCE;
...
PROTOTYPE* NewSimpleProto(int16_t N, CLUSTER* Cluster) {
  ...
  for (i = 0; i < N; i++) Proto->Mean[i] = Cluster->Mean[i];
...
bool Independent(PARAM_DESC* ParamDesc, int16_t N, FLOAT32* CoVariance,
  ...
  for (i = 0; i < N; i++, VARii += N + 1) {
    if (ParamDesc[i].NonEssential) continue;

    VARjj = VARii + N + 1;
    CoVariance = VARii + 1;
    for (j = i + 1; j < N; j++, CoVariance++, VARjj += N + 1) {
      if (ParamDesc[j].NonEssential) continue;
      ...
      if (CorrelationCoeff > Independence) return false;
    }
  }
  return true;
}  // Independent
...
BUCKETS* MakeBuckets(DISTRIBUTION Distribution, uint32_t SampleCount,
  ...
  Buckets->Confidence = Confidence;
  Buckets->Count = static_cast<uint32_t*>(
      Emalloc(Buckets->NumberOfBuckets * sizeof(uint32_t)));
  ...
  if (Symmetrical) {
    ...
    for (i = BUCKETTABLESIZE / 2; i < BUCKETTABLESIZE; i++) {
      ...
      if (Probability > NextBucketBoundary) {
        if (CurrentBucket < Buckets->NumberOfBuckets - 1) CurrentBucket++;
        NextBucketBoundary += BucketProbability;
      }
...
uint16_t OptimumNumberOfBuckets(uint32_t SampleCount) {
  ...
  if (SampleCount < kCountTable[0]) return kBucketsTable[0];
...
#define MINALPHA (1e-200)
{
  ...
  if (Odd(DegreesOfFreedom)) DegreesOfFreedom++;
...
void FillBuckets(BUCKETS* Buckets, CLUSTER* Cluster, uint16_t Dim,
  ...
  for (i = 0; i < Buckets->NumberOfBuckets; i++) Buckets->Count[i] = 0;
  ...
  if (StdDev == 0.0) {
    ...
    while ((Sample = NextSample(&SearchState)) != nullptr) {
      ...
      i++;
      if (i >= Buckets->NumberOfBuckets) i = 0;
    }
  } else {
...
uint16_t NormalBucket(PARAM_DESC* ParamDesc, FLOAT32 x, FLOAT32 Mean,
  ...
  if (X < 0) return 0;
  if (X > BUCKETTABLESIZE - 1) return ((uint16_t)(BUCKETTABLESIZE - 1));
...
uint16_t DegreesOfFreedom(DISTRIBUTION Distribution,
  ...
  if (Odd(AdjustedNumBuckets)) AdjustedNumBuckets++;
...
#define ILLEGAL_CHAR 2
{
  ...
  for (i = 0; i < NumFlags; i++) CharFlags[i] = FALSE;
