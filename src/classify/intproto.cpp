...
----------------------------------------------------------------------------*/
FLOAT32
BucketStart(int Bucket, FLOAT32 Offset, int NumBuckets);

FLOAT32
BucketEnd(int Bucket, FLOAT32 Offset, int NumBuckets);

void DoFill(FILL_SPEC* FillSpec, CLASS_PRUNER_STRUCT* Pruner,
            uint32_t ClassMask, uint32_t ClassCount, uint32_t WordIndex);

bool FillerDone(TABLE_FILLER* Filler);

void FillPPCircularBits(
    uint32_t ParamTable[NUM_PP_BUCKETS][WERDS_PER_PP_VECTOR], int Bit,
    FLOAT32 Center, FLOAT32 Spread, bool debug);
...
int AddIntProto(INT_CLASS Class) {
  ...
  if (Class->NumProtos >= MAX_NUM_PROTOS) return (NO_PROTO);
  ...
  if (Class->NumProtos > MaxNumIntProtosIn(Class)) {
    ...
    Class->ProtoLengths = (uint8_t*)Erealloc(
        Class->ProtoLengths, MaxNumIntProtosIn(Class) * sizeof(uint8_t));
...
void UpdateMatchDisplay() {
  if (IntMatchWindow != nullptr) IntMatchWindow->Update();
} /* ClearMatchDisplay */
#endif
 ...
 */
INT_CLASS
NewIntClass(int MaxNumProtos, int MaxNumConfigs) {
  INT_CLASS Class;
  PROTO_SET ProtoSet;
  int i;
...
NewIntTemplates() {
  ...
  for (i = 0; i < MAX_NUM_CLASSES; i++) ClassForClassId(T, i) = nullptr;
 ...
 */
INT_TEMPLATES
Classify::ReadIntTemplates(TFile* fp) {
  int i, j, w, x, y, z;
  int unicharset_size;
  int version_id = 0;
  INT_TEMPLATES Templates;
  ...
  if (version_id < 2) {
    ...
    max_class_id = 0;
    for (i = 0; i < Templates->NumClasses; i++)
      if (ClassIdFor[i] > max_class_id) max_class_id = ClassIdFor[i];
        ...
        for (y = 0; y < NUM_CP_BUCKETS; y++)
          for (z = 0; z < NUM_CP_BUCKETS; z++)
            for (w = 0; w < WERDS_PER_CP_VECTOR; w++) {
              if (TempClassPruner[i]->p[x][y][z][w] == 0) continue;
...
void DoFill(FILL_SPEC* FillSpec, CLASS_PRUNER_STRUCT* Pruner,
  ...
  X = FillSpec->X;
  if (X < 0) X = 0;
  if (X >= NUM_CP_BUCKETS) X = NUM_CP_BUCKETS - 1;

  if (FillSpec->YStart < 0) FillSpec->YStart = 0;
  if (FillSpec->YEnd >= NUM_CP_BUCKETS) FillSpec->YEnd = NUM_CP_BUCKETS - 1;
  ...
  for (Y = FillSpec->YStart; Y <= FillSpec->YEnd; Y++)
    for (Angle = FillSpec->AngleStart; TRUE;
      ...
      if (Angle == FillSpec->AngleEnd) break;
    }
} /* DoFill */
...
void FillPPCircularBits(
  ...
  if (FirstBucket < 0) FirstBucket += NUM_PP_BUCKETS;

  LastBucket = (int)floor((Center + Spread) * NUM_PP_BUCKETS);
  if (LastBucket >= NUM_PP_BUCKETS) LastBucket -= NUM_PP_BUCKETS;
...
void FillPPLinearBits(uint32_t ParamTable[NUM_PP_BUCKETS][WERDS_PER_PP_VECTOR],
  ...
  LastBucket = (int)floor((Center + Spread) * NUM_PP_BUCKETS);
  if (LastBucket >= NUM_PP_BUCKETS) LastBucket = NUM_PP_BUCKETS - 1;

  if (debug) tprintf("Linear fill from %d to %d", FirstBucket, LastBucket);
  for (i = FirstBucket; i <= LastBucket; i++) SET_BIT(ParamTable[i], Bit);
 ...
 */
CLASS_ID
Classify::GetClassToDebug(const char* Prompt, bool* adaptive_on,
