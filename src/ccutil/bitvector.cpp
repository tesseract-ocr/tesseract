...
bool BitVector::DeSerialize(bool swap, FILE* fp) {
  ...
  if (static_cast<int>(fread(array_, sizeof(*array_), wordlen, fp)) != wordlen)
    return false;
  if (swap) {
    for (int i = 0; i < wordlen; ++i) ReverseN(&array_[i], sizeof(array_[i]));
  }
  return true;
}
...
int BitVector::NextSetBit(int prev_bit) const {
  ...
  while (bit_index < word_end) {
    if (bit_index + 8 > next_bit && byte != 0) {
      ...
      if (byte != 0) return bit_index + lsb_index_[byte];
    }
    word >>= 8;
    bit_index += 8;
    byte = word & 0xff;
...
void BitVector::operator|=(const BitVector& other) {
  ...
  for (int w = 0; w < length; ++w) array_[w] |= other.array_[w];
}
void BitVector::operator&=(const BitVector& other) {
  int length = std::min(WordLength(), other.WordLength());
  for (int w = 0; w < length; ++w) array_[w] &= other.array_[w];
  for (int w = WordLength() - 1; w >= length; --w) array_[w] = 0;
}
void BitVector::operator^=(const BitVector& other) {
  int length = std::min(WordLength(), other.WordLength());
  for (int w = 0; w < length; ++w) array_[w] ^= other.array_[w];
...
void BitVector::SetSubtract(const BitVector& v1, const BitVector& v2) {
  ...
  for (int w = WordLength() - 1; w >= length; --w) array_[w] = v1.array_[w];
}
