...
double streamtofloat(FILE* s) {
  ...
  // Actual number parsing
  for (; c != EOF && (d = DigitValue(c, 10)) >= 0; c = fgetc(s)) v = v * 10 + d;
  if (c == '.') {
...
double strtofloat(const char* s) {
  ...
  // Actual number parsing
  for (; *s && (d = DigitValue(*s, 10)) >= 0; s++) v = v * 10 + d;
  if (*s == '.') {
...
static int tvfscanf(FILE* stream, const char* format, va_list ap) {
    ...
    switch (state) {
      case ST_NORMAL:
        if (ch == '%') {
          ...
          if (fgetc(stream) != ch) bail = BAIL_ERR;  // Match failure
        }
        break;
  ...
  if (bail == BAIL_EOF && !converted) converted = -1;  // Return EOF (-1)
