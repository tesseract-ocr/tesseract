...
#ifndef NDEBUG
    if (!current) NULL_DATA.error("ELIST2_ITERATOR::data", ABORT, nullptr);
    if (!list) NO_LIST.error("ELIST2_ITERATOR::data", ABORT, nullptr);
#endif
    return current;
  }
...
#ifndef NDEBUG
    if (!list) NO_LIST.error("ELIST2_ITERATOR::empty", ABORT, nullptr);
#endif
    return list->empty();
  }
...
inline void ELIST2_ITERATOR::add_after_then_move(  // element to add
  ...
  if (list->empty()) {
    ...
    if (current) {  // not extracted
      ...
      prev = current;
      if (current == list->last) list->last = new_element;
    } else {  // current extracted
      new_element->prev = prev;
      prev->next = new_element;
      if (ex_current_was_last) list->last = new_element;
      if (ex_current_was_cycle_pt) cycle_pt = new_element;
    }
  }
  current = new_element;
}
...
inline void ELIST2_ITERATOR::add_after_stay_put(  // element to add
  ...
  if (list->empty()) {
    ...
    if (current) {  // not extracted
      new_element->prev = current;
      current->next = new_element;
      if (prev == current) prev = new_element;
      if (current == list->last) list->last = new_element;
    } else {  // current extracted
      new_element->prev = prev;
      prev->next = new_element;
...
inline void ELIST2_ITERATOR::add_before_stay_put(  // element to add
  ...
  if (list->empty()) {
    ...
    if (current) {  // not extracted
      new_element->next = current;
      current->prev = new_element;
      if (next == current) next = new_element;
    } else {  // current extracted
      new_element->next = next;
      next->prev = new_element;
      if (ex_current_was_last) list->last = new_element;
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::add_list_after", ABORT, nullptr);
  if (!list_to_add)
  ...
  if (!list_to_add->empty()) {
    if (list->empty()) {
      ...
      if (current) {  // not extracted
        ...
        if (current == list->last) list->last = list_to_add->last;
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::add_list_before", ABORT, nullptr);
  if (!list_to_add)
  ...
  if (!list_to_add->empty()) {
    if (list->empty()) {
      ...
      if (current) {  // not extracted
        ...
        list_to_add->last->next = next;
        next->prev = list_to_add->last;
        if (ex_current_was_last) list->last = list_to_add->last;
        if (ex_current_was_cycle_pt) cycle_pt = prev->next;
      }
      current = prev->next;
      next = current->next;
    }
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::extract", ABORT, nullptr);
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::move_to_first", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::move_to_last", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::mark_cycle_pt", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::at_first", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::at_last", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::cycled_list", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::length", ABORT, nullptr);
#endif
...
inline void ELIST2_ITERATOR::sort(  // sort elements
    int comparator(                 // comparison routine
        const void*, const void*)) {
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::sort", ABORT, nullptr);
#endif
...
inline void ELIST2_ITERATOR::add_to_end(  // element to add
    ELIST2_LINK* new_element) {
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::add_to_end", ABORT, nullptr);
  if (!new_element)
   ...
   public:                                                                 \
    ...
    {                                                                      \
      ELIST2::internal_clear(&CLASSNAME##_zapper);                         \
                                                                           ...
                                                                           \
    ~CLASSNAME##_LIST() /* destructor */ { clear(); }                      \
                                                                           \
