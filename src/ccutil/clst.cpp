...
void CLIST::assign_to_sublist(  // to this list
  ...
  if (!empty()) LIST_NOT_EMPTY.error("CLIST.assign_to_sublist", ABORT, nullptr);
...
bool CLIST::add_sorted(int comparator(const void*, const void*), bool unique,
  ...
  if (last == nullptr || comparator(&last->data, &new_data) < 0) {
    ...
    for (it.mark_cycle_pt(); !it.cycled_list(); it.forward()) {
      void* data = it.data();
      if (data == new_data && unique) return false;
      if (comparator(&data, &new_data) > 0) break;
    }
    if (it.cycled_list())
...
void CLIST::set_subtract(int comparator(const void*, const void*), bool unique,
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("CLIST_ITERATOR::forward", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!current) NULL_DATA.error("CLIST_ITERATOR::forward", ABORT, nullptr);
  if (!next)
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("CLIST_ITERATOR::data_relative", ABORT, nullptr);
  if (list->empty())
...
#ifndef NDEBUG
  if (!ptr) NULL_DATA.error("CLIST_ITERATOR::data_relative", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("CLIST_ITERATOR::move_to_last", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("CLIST_ITERATOR::exchange", ABORT, nullptr);
  if (!other_it)
...
CLIST_LINK* CLIST_ITERATOR::extract_sublist(  // from this current
  ...
  if (!other_it)
    BAD_PARAMETER.error("CLIST_ITERATOR::extract_sublist", ABORT,
                        "other_it nullptr");
  if (!list) NO_LIST.error("CLIST_ITERATOR::extract_sublist", ABORT, nullptr);
