...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::forward", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!current) NULL_DATA.error("ELIST2_ITERATOR::forward", ABORT, nullptr);
#endif
...
ELIST2_LINK* ELIST2_ITERATOR::backward() {
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::backward", ABORT, nullptr);
#endif
  if (list->empty()) return nullptr;
  ...
  if (current) {  // not removed so
    ...
    if (ex_current_was_cycle_pt) cycle_pt = prev;
    current = prev;
  }

#ifndef NDEBUG
  if (!current) NULL_DATA.error("ELIST2_ITERATOR::backward", ABORT, nullptr);
  if (!prev)
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::data_relative", ABORT, nullptr);
  if (list->empty())
...
#ifndef NDEBUG
  if (!ptr) NULL_DATA.error("ELIST2_ITERATOR::data_relative", ABORT, nullptr);
#endif
...
#ifndef NDEBUG
  if (!list) NO_LIST.error("ELIST2_ITERATOR::exchange", ABORT, nullptr);
  if (!other_it)
...
ELIST2_LINK* ELIST2_ITERATOR::extract_sublist(  // from this current
  ...
  if (!other_it)
    BAD_PARAMETER.error("ELIST2_ITERATOR::extract_sublist", ABORT,
                        "other_it nullptr");
  if (!list) NO_LIST.error("ELIST2_ITERATOR::extract_sublist", ABORT, nullptr);
