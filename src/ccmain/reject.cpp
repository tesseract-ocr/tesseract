...
bool Tesseract::one_ell_conflict(WERD_RES* word_res, bool update_map) {
  ...
  /*
    ...
    is OK.
  */
  if (strpbrk(word, conflict_set_I_l_1.string()) == nullptr) return false;
  ...
  */
  ...
  if (!non_conflict_set_char) {
    if (update_map) reject_I_1_L(word_res);
    return true;
  }
  ...
  if (word_contains_non_1_digit(word, lengths)) {
    ...
    for (i = 0, offset = 0; word[offset] != '\0';
      ...
      if ((!allow_1s || (word[offset] != '1')) &&
          STRING(conflict_set_I_l_1).contains(word[offset])) {
        if (update_map) word_res->reject_map[i].setrej_1Il_conflict();
        conflict = true;
      }
    }
    return conflict;
...
int16_t Tesseract::alpha_count(const char* word, const char* word_lengths) {
  ...
  for (i = 0, offset = 0; word[offset] != '\0'; offset += word_lengths[i++]) {
    if (unicharset.get_isalpha(word + offset, word_lengths[i])) count++;
  }
  return count;
}
...
void Tesseract::dont_allow_1Il(WERD_RES* word) {
  ...
  if (!accepted_1Il) return;  // Nothing to worry about
...
bool Tesseract::repeated_nonalphanum_wd(WERD_RES* word, ROW* row) {
  ...
  if (word->best_choice->unichar_lengths().length() <= 1) return false;
