...
bool WordFeature::DeSerialize(FILE* fp) {
  return fread(&x_, fp) && fread(&y_, fp) && fread(&dir_, fp);
}
...
bool ImageData::DeSerialize(TFile* fp) {
  ...
  if (fp->FRead(&page_number_, 1) != 1) return false;
  ...
  inT8 vertical;
  if (fp->FRead(&vertical, 1) != 1) return false;
...
bool ImageData::SkipDeSerialize(TFile* fp) {
  ...
  inT32 page_number;
  if (fp->FRead(&page_number, 1) != 1) return false;
