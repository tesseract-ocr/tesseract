...
import { navigate as navigateUsingSegmentCache, NavigationResultTag } from '../../segment-cache/navigation';
...
export function navigateReducer(state, action) {
    ...
    if (process.env.__NEXT_PPR && process.env.__NEXT_CLIENT_SEGMENT_CACHE) {
        ...
        const result = navigateUsingSegmentCache(url, state.cache, state.tree, state.nextUrl);
        ...
        for (const normalizedFlightData of flightData){
            ...
            if (newTree !== null) {
                ...
                seedData && isRootRender && postponed) {
                    const task = updateCacheNodeOnNavigation(currentCache, currentTree, treePatch, seedData, head, isHeadPartial);
                    if (task !== null) {
