...
bool TrainingSample::DeSerialize(FILE* fp) {
  if (!fread(&class_id_, fp)) return false;
  if (!fread(&font_id_, fp)) return false;
  if (!fread(&page_num_, fp)) return false;
  if (!bounding_box_.DeSerialize(fp)) return false;
  if (!fread(&num_features_, fp)) return false;
  if (!fread(&num_micro_features_, fp))
    return false;
  if (!fread(&outline_length_, fp))
    return false;
  delete [] features_;
  features_ = new INT_FEATURE_STRUCT[num_features_];
  if (!fread(features_, fp, num_features_))
    return false;
  delete [] micro_features_;
  micro_features_ = new MicroFeature[num_micro_features_];
  return fread(&micro_features_[0][0], fp, num_micro_features_ * MFCount) &&
         fread(cn_feature_, fp, kNumCNParams) &&
         fread(geo_feature_, fp, GeoCount);
}
